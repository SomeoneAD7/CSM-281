...
def print_matrix(matrix, name):
    ...
    for row in matrix:
        formatted = "  [ " + "  ".join(f"{val:6.4g}" for val in row) + " ]"
        print(formatted)
    print()

def print_vector(vector, name):
    """Pretty-print a 1D vector as a column vector."""
    print(f"  {name} ({len(vector)} x 1):")
    for val in vector:
        print(f"  [ {val:6.4g} ]")
    print()

print("=" * 60)
print("         MATRICES AND VECTORS USED IN THIS SOLUTION")
print("=" * 60)
print()
print_matrix(A, "Matrix A  —  4×4 Symmetric Positive Definite")
print_matrix(B, "Matrix B  —  3×2 Full-Rank")
print_vector(b, "Vector b  —  RHS for Ax = b")
print_vector(c, "Vector c  —  RHS for Bx = c")
